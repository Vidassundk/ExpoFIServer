"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ListCourseBookmarksUsecase = void 0;
class ListCourseBookmarksUsecase {
    constructor(bookmarkRepo) {
        this.bookmarkRepo = bookmarkRepo;
    }
    // Method to list bookmarks by user
    exec(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.bookmarkRepo.listBookmarkByUserId(userId);
        });
    }
    // Method to add a bookmark
    addBookmark(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.bookmarkRepo.addBookmark(userId, courseId);
        });
    }
    // Method to remove a bookmark
    removeBookmark(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.bookmarkRepo.removeBookmark(userId, courseId);
        });
    }
    // Method to check if a course is bookmarked by a user
    isBookmarked(userId, courseId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.bookmarkRepo.isBookmarked(userId, courseId);
        });
    }
}
exports.ListCourseBookmarksUsecase = ListCourseBookmarksUsecase;
//# sourceMappingURL=ListCourseBookmarksUsecase.js.map